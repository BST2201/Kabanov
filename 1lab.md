# Выполнил студент группы БСТ2201 Кабанов Константин Александрович
...
## 1 задание
```python
print("\nЗадание №1\nHello, World! My name is Konstantin.")
```
...
selection_time = round((time.time()-start_time)*1000) #Фиксирую время работы алгоритма
```
...
insertion_time = round((time.time()-start_time)*1000) #Фиксирую время работы алгоритма
```
...
bubble_time = round((time.time()-start_time)*1000) #Фиксирую время работы алгоритма
```
...
shell_time = round((time.time()-start_time)*1000) #Фиксирую время работы алгоритма
```

Быстрая сортировка (применён метод рекурсии)
```python
...
quicksort_time = round((time.time()-start_time)*1000) #Фиксирую время работы алгоритма
```
...
tournament_time = round((time.time()-start_time)*1000) #Фиксирую время работы алгоритма
```
...
base_time = round((time.time()-start_time)*1000) #Фиксирую время работы алгоритма
```

Сравнение ранее зафиксированных временных результатов
```python
print("\nИтоги:\n")
...
1. Ознакомился с базовыми принципами синтаксиса языка программирования python
2. Изучил простейший метод создания рекурсии
3. Изучил метод получения информации от пользователя и вывод пользователю итогов выполнения алгоритма
4. Изучил теорию по нескольким методам сортировки матриц и массивов
5. Сравнил время работы нескольких методов сортировки